...
def _load(d: str):
    return load_person_data(PERSON, Path(d))
...
with st.sidebar:
    ...
    up_bank = st.file_uploader("Bank statement CSVs", type="csv",
                                accept_multiple_files=True, key="y_bank")
    ...
    # BUG-1 fix: filter defaults to options actually present
    safe_exc = [c for c in EXCLUDE_FROM_LIFESTYLE if c in all_cats]
    exclude_cats = st.multiselect("Exclude categories", all_cats, default=safe_exc)
...
st.info(
    "💡 Bank transactions are mostly ATM cash withdrawals. "
    "The credit card captures the main visible spending categories.",
    icon="ℹ️",
)
...
st.plotly_chart(waterfall_chart(
    filtered.groupby("YearMonth")["Amount"].sum().sort_index(), PERSON
...
st.subheader("Spend heatmap (day of month × calendar month)")
...
st.download_button(
    ...
    f"{PERSON}_spending_filtered.csv", "text/csv",
)
